...
def index(request):
    if request.method == 'POST':
        ...
        # url = 'http://www.rediff.com/rss/cricketrss.xml'
...
def Score(request):
    ...
    # r = session.get(link)
    # info = feerecdparser.parse(r)

    # # score = r.html.find(".cscore_score")
    # # info = r.html.find(".cscore_info-overview", first=True).text
    # # team = r.html.find(".cscore_name--long")
    # # inning = r.html.find(".accordion-header h4", first=True)
    # # # record = r.html.find(".batsmen", first=True)
    # batsman = r.html.find(".collapse")
    #
    # articles = r.html.find("article")
    # for article in articles:
    #     scorecard_summary = article.find(".scorecard")
